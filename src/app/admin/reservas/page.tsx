...
import { obtenerReservas, eliminarReserva, actualizarReserva } from '@/app/lib/reservas';
...
export default function PanelReservas() {
  ...
  const cargarReservas = async () => {
    ...
    try {
      ...
      const hoy = new Date();
      const reservasValidas = data.filter(r => new Date(r.fechaSalida) >= hoy);
  ...
  const getBadgeVariant = (unidad: Unidad) => {
    switch (unidad) {
      case 'este': return 'default';
      case 'oeste': return 'secondary';
      case 'cabana': return 'destructive';
      case 'camping': return 'outline';
            ...
            <Table>
              ...
              <TableBody>
                {loading && reservas.length === 0 ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={7} className="text-center">
                      No hay reservas {filtroUnidad ? `para ${filtroUnidad}` : ''}
                    </TableCell>
                  </TableRow>
                ) : (
                  reservasPaginadas.map((reserva) => (
                    <TableRow key={reserva.id}>
                      <TableCell className="font-medium">{reserva.nombreCompleto}</TableCell>
                      <TableCell>
                        <div>{reserva.email}</div>
                        <div className="text-sm text-muted-foreground">{reserva.telefono}</div>
                      </TableCell>
                      <TableCell>
                        <Badge variant={getBadgeVariant(reserva.unidad)} className="capitalize">
                          {reserva.unidad}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        {reserva.tipoReserva === 'cama_individual' && 'Cama Individual'}
                        {reserva.tipoReserva === 'habitacion_completa' && 'Habitación Completa'}
                        {reserva.tipoReserva === 'cabana_completa' && 'Cabaña Completa'}
                      ...
                      <TableCell className="text-center">{reserva.cantidadPersonas}</TableCell>
                      <TableCell>
          ...
          {reservasFiltradas.length > 0 && (
            <div className="flex flex-col md:flex-row justify-between items-center mt-4 gap-4">
              <div className="text-sm text-muted-foreground">
                ...
                {Math.min(paginaActual * reservasPorPagina, reservasFiltradas.length)} de {reservasFiltradas.length} reservas
              </div>
              <Pagination>
                <PaginationContent>
                  <PaginationItem>
